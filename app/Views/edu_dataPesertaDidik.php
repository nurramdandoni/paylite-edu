        ...
        async function getSiswa(){
          ...
          const requestOptions = {
                  ...
                  body: JSON.stringify(postSiswa), // Mengubah data menjadi bentuk JSON
                    ...
                    for(item of data.data){
                      ...
                      <td>
                        <div class="d-flex px-2 py-1">
                          ...
                          <div class="d-flex flex-column justify-content-center">
                            <h6 class="mb-0 text-sm">`+item.nisn+`</h6>
                          </div>
                        </div>
                      </td>
                      <td>
                      ...
                      <td class="align-middle">
                        <a data-toggle="modal" data-target="#exampleModal" onclick="formEdit('`+item.siswa_id+`')" class="text-secondary font-weight-bold text-xs" data-toggle="tooltip" data-original-title="Edit user">
                          Edit
                        </a>
                      </td>
                      <td class="align-middle">
                ...
                async function formEdit(id){
                  ...
                  <div class="form-group">
                    <label for="namaSiswa">Nama Peserta Didik</label>
                    <input type="text" value="`+dt.data.nama_siswa+`" class="form-control" id="namaSiswa" aria-describedby="namaS" placeholder="Nama Siswa. Ex. Kenziro">
                    <small id="namaS" class="form-text text-muted">Masukan Nama Peserta Didik</small>
                  </div>
                  <div class="form-group">
                    <label for="jenisKelamin">Jenis Kelamin</label>
                ...
                async function getExist(id){
                    const data = await fetchData('https://api.paylite.co.id/siswa/'+id+'');
                    return data;
                }
                async function cekExist(nisn){
                ...
                async function insertDataSiswa(dataPost){
                  const requestOptions = {
                  method: 'POST', // Metode permintaan
                  headers: {
                            'Content-Type': 'application/json', // Jenis konten yang dikirim
                  ...
                  body: JSON.stringify(dataPost), // Mengubah data menjadi bentuk JSON
                    ...
                    const data = await fetchData('https://api.paylite.co.id/siswa', requestOptions);
                    return data;
                }
                async function updateDataSiswa(id,dataPost){
                  const requestOptions = {
                  ...
                  body: JSON.stringify(dataPost), // Mengubah data menjadi bentuk JSON
                    ...
                    const data = await fetchData('https://api.paylite.co.id/siswa/'+id+'', requestOptions);
                    return data;
                }
                ...
                async function modalButtonAction(){
                  ...
                  const id = $("#idData").val();
                  const nisn = $("#nisSiswa").val();
                  const nama_siswa = $("#namaSiswa").val();
                  ...
                  const hasilCek = await cekExist(nisn);
                    console.log(hasilCek);
                  if(hasilCek.data.length > 0){
                    console.log("id nya : ", id);
                    if(id != undefined){
                      const update = await updateDataSiswa(id,postData);
                    ...
                    if(tipe == "add"){
                      ...
                      const inserted = await insertDataSiswa(postData);
                      if(inserted){
